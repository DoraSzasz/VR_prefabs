	...
	SubShader 
	{
		...
		{ 
			...
			float map(float3 position)
			{
				...
				float wavesb			= voronoi(noise + 5. - position.xz * 32. + time * -3.15) * .32;
			
				float waves				= lerp(wavesa, wavesb, (cos(sin(_Time.x)*wavesa*wavesb)*.5+.5) * .5);
			...
			v2f vert (appdata v)
			{
				...
				float surface_scatter	= clamp(waves * waves * 512. * depth * _Waves, 0., 1.);
				float sub_scatter		= .25 * distribution(.5, half_exposure);
				...
				//gamma adjustment
				o.color.xyz				= pow(o.color.xyz * 1.2, 1.6);
